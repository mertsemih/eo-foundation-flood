...
Table~\ref{tab:full} reports all five models at $f=1$; the study comprises \nruns{} training runs in total. \todo{prose once the LoRA seeds are in}

\begin{table}[t]
\centering
\caption{All training labels ($f=1$). Water IoU on the test split and on the held-out Bolivia split, mean $\pm$ std over three seeds ($\dagger$: single seed). GPU minutes on an RTX~3070~Ti (8\,GB).}
\label{tab:full}
\small
\input{tables/table_full}
\end{table}

\begin{table}[t]
\centering
\caption{Test water IoU vs.\ training-label fraction (number of chips in parentheses). Best per row in bold.}
\label{tab:sweep}
\small
\input{tables/table_sweep_test}
\end{table}

\begin{table}[t]
\centering
\caption{Bolivia (held-out region) water IoU vs.\ training-label fraction.}
\label{tab:sweep_bolivia}
\small
\input{tables/table_sweep_bolivia}
\end{table}

\begin{table}[t]
\centering
\caption{Ablations at $f=1$: LoRA rank, LoRA target modules, and decoder ($\dagger$: single seed).}
\label{tab:ablation}
\small
